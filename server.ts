    ...
    if (count === 0) {
      await prisma.servicio.createMany({
        data: [
          ...
          { nombre: 'Coloración', descripcion: 'Cambio de color con tinte profesional', duracion_minutos: 60, precio: 60, activo: true },
          { nombre: 'Tratamiento Capilar', descripcion: 'Hidratación y regeneración del cabello', duracion_minutos: 45, precio: 50, activo: true },
...
app.post('/api/citas', async (req, res) => {
  try {
    ...
    // Crear o buscar cliente usando Prisma (SQLite local - más confiable)
